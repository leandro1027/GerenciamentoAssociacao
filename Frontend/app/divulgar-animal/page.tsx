...
const RadioPill = ({ label, name, value, checked, onChange, icon }: { label: string, name: string, value: string, checked: boolean, onChange: (value: string) => void, icon?: React.ReactNode }) => (
  <motion.label 
    ...
    whileTap={{ scale: 0.98 }}
    className={`cursor-pointer px-4 sm:px-6 py-3 sm:py-4 rounded-2xl text-sm sm:text-base font-semibold border-2 transition-all duration-300 flex items-center justify-center gap-2 ${
      checked 
...
const InfoCard = ({ icon, title, description, type = 'info' }: { icon: React.ReactNode, title: string, description: string, type?: 'info' | 'warning' | 'success' }) => {
  ...
  return (
    <motion.div 
      ...
      className={`p-3 sm:p-4 rounded-2xl border-2 ${typeStyles[type]} flex items-start gap-3`}
    >
  ...
  return (
    <main className="flex-grow bg-gradient-to-br from-amber-50 via-white to-orange-50 py-4 sm:py-8 lg:py-12">
      <div className="max-w-6xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        >
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent mb-3 sm:mb-4">
            Divulgar Animal
          </h1>
          <p className="text-gray-600 text-sm sm:text-base max-w-2xl mx-auto px-4">
            Ajude um animal a encontrar um lar amoroso. Preencha as informações abaixo com cuidado.
          </p>
        </motion.header>
        ...
        >
          <div className="bg-white/80 backdrop-blur-sm p-3 sm:p-4 rounded-2xl shadow-lg border border-amber-200 w-full max-w-md sm:max-w-none">
            <div className="flex items-center justify-between sm:justify-center sm:gap-8">
              {[
                ...
                <div key={step.number} className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4">
                  <div className="flex items-center gap-2 sm:gap-4">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center font-semibold text-sm sm:text-base transition-all duration-300 ${
                      step.active 
                    ...
                    <span className={`font-semibold text-xs sm:text-sm ${step.active ? 'text-amber-700' : 'text-gray-500'} hidden sm:block`}>
                      {step.label}
                    </span>
                  </div>
                  {index < 2 && (
                    <div className={`hidden sm:block w-8 sm:w-12 h-1 rounded-full ${step.active ? 'bg-amber-500' : 'bg-gray-300'}`} />
                  )}
                </div>
              ))}
            </div>
            {/* Labels mobile abaixo dos números */}
            <div className="flex justify-between mt-2 sm:hidden">
              {[
                ...
                <span 
                  key={step.number} 
                  className={`font-semibold text-xs ${step.active ? 'text-amber-700' : 'text-gray-500'}`}
                >
                  {step.label}
                </span>
              ))}
        ...
        <motion.form 
          ...
          className="bg-white p-4 sm:p-6 lg:p-8 xl:p-10 rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl space-y-8 sm:space-y-12 border border-amber-100"
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
            ...
            >
              <fieldset>
                ...
                <motion.div 
                  className={`relative flex flex-col justify-center items-center p-4 sm:p-6 lg:p-8 border-3 border-dashed rounded-2xl sm:rounded-3xl transition-all duration-300 h-full min-h-[280px] sm:min-h-[350px] ${
                    isDragging 
                  ...
                  <AnimatePresence mode="wait">
                    {previewImage ? (
                      ...
                      >
                        ...
                        >
                          <label htmlFor="file" className="cursor-pointer">
                            <Button 
                              variant="outline" 
                              className="bg-white/90 backdrop-blur-sm border-amber-300 text-amber-700 hover:bg-white text-sm sm:text-base py-2 px-4"
                            >
                      ...
                      >
                        ...
                        <p className="text-gray-500 text-xs sm:text-sm mb-3 sm:mb-4">ou arraste e solte aqui</p>
          ...
          >
            <Button 
              type="submit" 
              isLoading={isLoading}
              className="bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-base sm:text-lg px-6 sm:px-12 py-3 sm:py-4 rounded-xl sm:rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 text-white font-semibold w-full sm:w-auto"
              disabled={!file}
            >
              {isLoading ? (
                <>Enviando...</>
