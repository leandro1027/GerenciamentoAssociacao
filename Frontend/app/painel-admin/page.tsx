...
export default function AdminPanelPage() {
  ...
  const handleCreateSlide = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!slideFile) return alert('Por favor, selecione uma imagem.');
    ...
    try {
      ...
      setSlideTitle(''); setSlideSubtitle(''); setSlideFile(null);
  ...
  const Sidebar = () => (
    <aside className={`bg-stone-800 text-white flex flex-col flex-shrink-0 transition-all duration-500 ease-in-out ${isSidebarOpen ? 'w-64 p-4' : 'w-0 p-0 overflow-hidden'}`}>
        ...
        <div className="mt-auto">
            <Link href="/" className="block text-center p-3 rounded-lg bg-stone-700 hover:bg-stone-600 transition-colors whitespace-nowrap">Sair do Painel</Link>
        </div>
    </aside>
  );
                            ...
                            <div className="bg-white rounded-xl shadow p-6">
                                <form onSubmit={handleCreateSlide} className="mb-6 p-4 border rounded-lg space-y-3 bg-gray-50">
                                    ...
                                    <div>
                                        ...
                                        <input id="slide-file-input" type="file" accept="image/*" onChange={(e) => { if (e.target.files) setSlideFile(e.target.files[0]); }} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" required />
                                    </div>
